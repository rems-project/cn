...
module Flags = struct
  ...
  let solver_inc_timeout =
    let doc = "Timeout after which a non-incremental SMT solver replaces the incremental solver" in
    Arg.(value & opt (some int) !Solver.inc_timeout & info [ "incremental-solver-timeout" ] ~doc)
