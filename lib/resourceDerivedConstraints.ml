...
let derived_lc1 ((resource : Request.t), O output) =
  ...
  | P { name; pointer; iargs = [] }
    when !MT.use_vip && Request.(equal_name name Alloc.Predicate.name) ->
    let module H = Alloc.History in
...
let derived_lc2 ((resource : Request.t), _) ((resource' : Request.t), _) =
