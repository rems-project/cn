  ...
  (match pe_ with
    ...
    | PEif (pe, e1, e2) ->
      ...
      let check_path cond e = 
        pure_persist_logical_variables LC.(
          let@ () = add_c loc (LC.T cond) in 
          let@ v = check_pexpr e in
          let@ s' = get_typing_context () in
          let lcs = 
            Set.map (impl loc cond) (Set.diff s'.constraints s.constraints)
          in
    ...
    let check =
      let rec aux args_acc args gargs ftyp k =
        ...
        | arg :: args, _, Computational ((s, bt), info, ftyp) ->
          let@ () = WellTyped.ensure_base_type (fst info) ~expect:bt (Mu.bt_of_pexpr arg) in
