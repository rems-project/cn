...
module Make (AD : Domain.T) = struct
  ...
  module Integer = struct
    ...
    (* Collect constraints for variable x from a term, returning constraints *)
    let rec collect_and_extract_constraints (vars : Sym.Set.t) (x : Sym.t) (gt : Term.t)
      ...
      | `Pick gts ->
        ...
        let constraints = List.map (collect_and_extract_constraints vars x) gts in
      ...
      | `ITE (it_if, gt_then, gt_else) ->
        let rep_then =
          ...
          match Rep.of_it x it_if with
          | Some rep_if ->
            (* Found a constraint, merge *)
            Rep.intersect rep_if rep_then
          | None -> rep_then
        in
    ...
    let mk_arbitrary_specialized (v : Rep.t) (bt : BT.t) (loc : Locations.t) : Term.t =
      ...
      then (
        ...
        Term.arbitrary_ () bt loc (* No constraints, keep as Arbitrary *)
