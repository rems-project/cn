...
let synthesize
  ...
  let ctx = Stage4.transform paused ctx in
  ...
  if TestGenConfig.is_symbolic_enabled () then
    ...
    let ctx = Stage6.transform ctx in
    debug_stage "Stage 6" (ctx |> Stage6.Ctx.pp |> Pp.plain ~width:80);
    let module Stage7 = Stage7.Make (AD) in
    Stage7.transform sigma prog5 ctx
