...
module Make (AD : Domain.T) = struct
  ...
  let transform_gt (gt : Stage5.Term.t) : Term.t =
    let rec aux (vars : Sym.t list) (path_vars : Sym.Set.t) (gt : Stage5.Term.t) : Term.t =
  ...
  let transform_gd
        ({ filename; recursive; spec; name; iargs; oarg; c_types; body } : Stage5.Def.t)
    : Def.t
    =
    { filename; recursive; spec; name; iargs; oarg; c_types; body = body |> transform_gt }


  let transform (ctx : Stage5.Ctx.t) : Ctx.t = List.map_snd transform_gd ctx
end
