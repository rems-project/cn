...
module Interpreter : Domain.Interpreter = struct
  module AD = struct
    let name = name

    type s = t [@@deriving eq, ord]

    type t = s [@@deriving eq, ord]
  ...
  let abs_stmt
    ...
    | `Asgn ((it_addr, sct), _, _) | `AsgnElab (_, ((_, it_addr), sct), _, _) ->
      let rec pointer_and_offset (it : IndexTerms.t) : (Sym.t * int) option =
        let open Option in
        match it with
        ...
        | IT (Sym x, _, _) | IT (Cast (_, IT (Sym x, _, _)), _, _) -> return (x, 0)
        | _ -> None
      in
