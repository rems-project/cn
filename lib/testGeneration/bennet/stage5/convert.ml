...
module Make (AD : Domain.T) = struct
  ...
  let count_recursive_calls (syms : Sym.Set.t) (gr : Stage4.Term.t) : int =
  ...
  let size_recursive_calls (syms : Sym.Set.t) (size : int) (gr : Stage4.Term.t)
    : Term.t * Sym.Set.t
    =
    let rec aux (gr : Stage4.Term.t) : Term.t * Sym.Set.t =
  ...
  let transform_gt (syms : Sym.Set.t) (gr : Stage4.Term.t) : Term.t =
    let rec aux (path_vars : Sym.Set.t) (gr : Stage4.Term.t) : Term.t =
      let (Annot (gr_, (), bt, loc)) = gr in
      match gr_ with
      | `ITE (cond, t, f) ->
