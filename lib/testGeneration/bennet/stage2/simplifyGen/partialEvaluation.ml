...
module Make (AD : Domain.T) = struct
  ...
  module MakeTerm = struct
    ...
    let eval_term_generic
      ...
      | Binop ((BW_CLZ_Z | BW_CTZ_Z), _it1, _it2) -> Error "todo: CLZ_Z and CTZ_Z"
      | Binop ((BW_FFS_Z | BW_FLS_Z), _it1, _it2) -> Error "todo: FFS_Z and FLS_Z"
      | Binop (LT, it1, it2) ->
        let@ it1 = eval_aux it1 in
    ...
    let eval_term_strictly (prog5 : unit Mucore.file) (it : T.t) : (T.t, string) result =
      let rec eval_aux (it : T.t) : (T.t, string) result =
        ...
        | HasAllocId _ | SizeOf _ | OffsetOf _ | Nil _ | Head _ | Tail _ | Representable _
        | Good _ | Aligned _ | MapGet _ | MapDef _ | Apply _ | Match _ ->
    ...
    let eval_term_lazily (prog5 : unit Mucore.file) (it : T.t) : (T.t, string) result =
      let rec eval_aux (it : T.t) : (T.t, string) result =
        ...
        | Head _ | Tail _ | Representable _ | Good _ | Aligned _ | MapGet _ | MapDef _
        | Apply _ | Let _ | Match _ ->
          eval_term_generic eval_aux prog5 it
        (* Lazy *)
        | Tuple _ | Struct _ | Record _ | Constructor _ | Cons _ | MapConst _ | MapSet _
          ->
