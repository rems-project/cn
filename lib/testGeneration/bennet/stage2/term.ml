...
let let_star_ (((x, gt1), gt2) : (Sym.t * t) * t) (loc : Locations.t) : t =
  GT (`LetStar ((x, gt1), gt2), basetype gt2, loc)
...
let is_let_ (gt_ : t_) : bool = match gt_ with `LetStar _ -> true | _ -> false
...
let rec contains_call (gt : t) : bool =
  ...
  | `LetStar ((_, gt_inner), gt_rest) -> contains_call gt_inner || contains_call gt_rest
