...
let debug_bounds name constraint_term (x, bt) =
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, bt) constraint_term in
  Printf.printf
    "%s: lower=%s, upper=%s\n"
    name
...
let test_bounds_simple_eq _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) eq_5 in
...
let test_bounds_simple_le _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) le_10 in
...
let test_bounds_simple_ge _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) ge_3 in
...
let test_bounds_conjunction _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) conj in
  assert_bool
...
let test_bounds_disjunction _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) disj in
...
let test_bounds_irrelevant_variable _ =
  ...
  let lower, upper = Cn.TermBounds.get_bounds_opt (x, test_bt) y_le_5 in
  assert_bool
...
let bounds_consistency_prop =
  Test.make
    ...
    (fun (lower_val, upper_val, eq_val) ->
       ...
       let eq_lower, eq_upper = Cn.TermBounds.get_bounds_opt (x, test_bt) eq_constraint in
...
let random_inequality_bounds_prop =
  Test.make ~count:50 ~name:"random inequality bounds" (int_range (-50) 50) (fun value ->
    ...
    let le_lower, le_upper = Cn.TermBounds.get_bounds_opt (x, test_bt) le_constraint in
    let le_correct =
    ...
    let ge_lower, ge_upper = Cn.TermBounds.get_bounds_opt (x, test_bt) ge_constraint in
    let ge_correct =
