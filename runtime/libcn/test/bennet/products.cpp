...
TEST_F(LibBennet, CongrOwnershipWintArbitrary_TopCongrBoundedWint_Aligned) {
  ...
  // exposed when P5.2 made this sampler reachable.
...
TEST_F(LibBennet, ProductPin_CongrWint_Uintptr_BiasPath) {
  ...
  // draw interleaves with the joint index draw. Goldens captured after P5.1
  // unified the index draw on bennet_arbitrary_wint_of (the sampler is
  // unreachable from generated code until P5.2 wires it up).
  bennet_srand(PRODUCT_PIN_SEED);
  bennet_set_size(20);
  bennet_rand_alloc_free_all();
...
TEST_F(LibBennet, ProductPin_CongrWint_Uintptr_ConstrainedIndex) {
  ...
  // draw - pure joint index draw (post-P5.1 unification goldens).
  bennet_srand(PRODUCT_PIN_SEED);
  bennet_set_size(20);
  bennet_rand_alloc_free_all();
