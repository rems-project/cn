 ...
 *
 * P4.0 of doc/RUNTIME-ABSINT.md pinned the legacy walker behavior; the P4.1
...
TEST_F(LibBennet, OwnershipFwdSymNonLocTagged) {
  // DIVERGENCE WITNESS (flipped by the P4.1 port): the legacy forward SYM
...
TEST_F(LibBennet, OwnershipBwdDepositIgnoresTarget) {
  // DEPOSIT WITNESS (flipped by P6.6): the target parameter is vestigial -
...
TEST_F(LibBennet, OwnershipAssumeAndFalseEqNoRefine) {
  ...
  // (the P6.3 join rules refine nothing here).
  cn_bump_frame_id frame = cn_bump_get_frame_id();
  auto* state = bennet_absint_state_create();
  cn_sym p = cn_sym_from_string("p");
  cn_sym q = cn_sym_from_string("q");
...
TEST_F(LibBennet, OwnershipAssumeOrTrueJoinsEqRefinement) {
  // or(eq, eq) true (the former D1 gap): each branch refines p and q to the
...
TEST_F(LibBennet, OwnershipAssumeEqIteDistinctArms) {
  ...
  // so p and q stay top and only r is refined. (Value-stable across the P4.1
...
TEST_F(LibBennet, OwnershipAssumeEqIteSameSymArms) {
  // DIVERGENCE WITNESS (flipped by the P4.1 port): ite(c, &p->a, &p->b) == r.
...
TEST_F(LibBennet, OwnershipAssumeEqBottomMet) {
  // DIVERGENCE WITNESS (flipped by the P4.1 port): ite(c, p, q) == r with r
...
// over the engine's deposit walk since P6.6 - blame now crosses
...
TEST_F(LibBennet, OwnershipPropagateIteOneArmJoinsTop) {
  ...
  // one-arm syms still pick up nothing (value-stable across P6.6).
  cn_bump_frame_id frame = cn_bump_get_frame_id();
  auto* state = bennet_absint_state_create();
  cn_sym p = cn_sym_from_string("p");
  cn_sym q = cn_sym_from_string("q");
...
TEST_F(LibBennet, OwnershipPropagateIteSameSymBothArms) {
  // DEPOSIT WITNESS (new capability in P6.6): blame crosses conditionals.
