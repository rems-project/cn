...
const TermEntry kNumericOneSym[] = {
    ...
    // P6.5 node coverage: WRAPI (modular conversion; == CAST for absint) and
...
const TermEntry kCondOneSym[] = {
    ...
    // Unop inversion witnesses (P6.1): the legacy wint backward-unop default
...
const TermEntry kCondTwoSym[] = {
    ...
    // Binop top-fallback witness (P6.1): with both syms unconstrained, the
 ...
 * evaluator's signed semantics turn out to differ (doc P0 risk note).
 *---------------------------------------------------------------------------*/
...
TEST_F(AbsintOracle, GoldenCardinalities) {
  ...
  // tnum assume through structure (D2 fix witness): (x & 0xF0) == 0x50 from
  // top refines x to {0x5?}, i.e. value 0x50 with mask 0x0F -> 16 values.
  // Before the D2 fix the SYM-only application dropped this refinement
  // entirely (256 values).
  {
    bennet_absint_state* st = bennet_absint_state_create();
  ...
  // wint assume through NEGATE (P6.1 fix witness): (-x == 5) from top refines
  ...
  // OR-true join (P6.3 / D1 fix witnesses): x==3 || x==7 from top. wint
  ...
  // constants to 4Z+3 (64 values). All were 256 pre-P6.3.
  {
  ...
  // AND-false join (P6.3 / D1 fix witness): !(x<100 && x<50) puts x in the
  ...
  // WRAPI assume (P6.5 coverage witness): wrapi8(x+3) == 5 refines x to {2}
  ...
  // LET forward (P6.5 coverage witness): let t = x+3 in t*2 with x in
  ...
  // congr: x * 4 from top (C3 mul-stride): the general gcd formula recovers 4Z+0
  ...
  // congr: (4Z+1) ^ 2 (C3 bitwise, formerly default->top): XOR preserves the
  ...
  // congr: ~(4Z+1) (C3 BW_Compl, formerly default->top): ~x = -x-1 preserves the
  ...
  // congr: {7} % 4 (C3 singleton-dividend mod): exact remainder {3} -> 1 value
