...
TEST_F(LibBennet, WIntBackwardBothSidesSameSymMeets) {
  // DEPOSIT WITNESS (new in P6.6): x - x with output {2} and x in [0,10].
...
TEST_F(LibBennet, WIntBackwardBottomOutputBottomsAllSyms) {
  // DEPOSIT WITNESS (new in P6.6): a bottom output bottoms every sym of the
...
TEST_F(LibBennet, WIntAssumeOrTrueJoinsHull) {
  // x==3 || x==7 assumed true (the former D1 gap: wint's AND/OR handling was
