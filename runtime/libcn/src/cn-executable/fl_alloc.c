...
static inline void fl_coalesce(free_block_header* fl) {
  ...
  if (!prev_used && !next_used) {
    // Assert that prev_free and next_free are not NULL
    assert(prev_free != NULL && "prev_free should not be NULL when !prev_used");
    assert(next_free != NULL && "next_free should not be NULL when !next_used");
    if (prev_free_prev) {
    ...
    // Assert that next_free is not NULL
    assert(next_free != NULL && "next_free should not be NULL when !next_used");
    if (prev_free) {
    ...
    // Assert that prev_free is not NULL
    assert(prev_free != NULL && "prev_free should not be NULL when !prev_used");
    if (prev_free_prev) {
      prev_free->prev = fl_offset(&prev_free_prev->header);
      prev_free_prev->next = fl_offset(&prev_free->header);
