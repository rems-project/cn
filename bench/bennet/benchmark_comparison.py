...
def parse_benchmark_output(output):
    ...
    for line in lines:
        ...
        result_match = re.match(r'(PASSED|FAILED): (.+) \(took ([\d.]+)s\)', line)
        if result_match:
        ...
        config_match = re.search(r'- all configs (?:passed|failed)\. \(([^)]+)\)', line)
...
def run_benchmarks(symbolic=False):
    ...
    if result.returncode not in [0, 1]:  # 0 = all passed, 1 = some failed
        print(f"Benchmark command failed unexpectedly with code {result.returncode}")
        print(f"stdout: {result.stdout}")
        print(f"stderr: {result.stderr}")
        return None
...
def run_benchmarks_in_dir(directory, symbolic=False):
    ...
    result = run_command(cmd, cwd=directory, check=False)  # Don't fail on test failures
...
def compare_results(current_results, upstream_results):
    ...
    for test_file in sorted(set(current_data.keys()) | set(upstream_data.keys())):
        ...
        print(f"{test_file:<40} {current_time:<10.3f} {upstream_time:<10.3f} {diff:<+10.3f} {change_str}")
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark CN test performance against upstream/main')
    ...
    current_filename = os.path.join(args.results_dir, f'benchmark_current_{mode}_{timestamp[:19].replace(":", "-")}.json')
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            upstream_url_result = run_command(["git", "remote", "get-url", "upstream"])
            upstream_url = upstream_url_result.stdout.strip()
        except subprocess.CalledProcessError:
            print("No 'upstream' remote found, using 'origin'")
            upstream_url_result = run_command(["git", "remote", "get-url", "origin"])
            upstream_url = upstream_url_result.stdout.strip()
        ...
        # Get upstream commit
        upstream_commit = run_command(["git", "rev-parse", "HEAD"], cwd=upstream_dir).stdout.strip()
        ...
        upstream_benchmark_data = run_benchmarks_in_dir(upstream_dir, args.symbolic)
        ...
        upstream_filename = os.path.join(args.results_dir, f'benchmark_upstream_{mode}_{timestamp[:19].replace(":", "-")}.json')
